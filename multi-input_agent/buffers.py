...
class DictReplayBuffer(ReplayBuffer):
    ...
    def add(
        ...
        for key in self.next_observations.keys():
            ...
            if (done): #jmv
                self.next_observations[key][self.pos] = np.array(next_obs[0][key]).copy()
            else:
        ...
        if self.handle_timeout_termination:
            #print("np    ", np.array([info.get("TimeLimit.truncated", False) for info in infos]))
            #print("self.timeouts[self.pos]", self.timeouts[self.pos])
            #self.timeouts[self.pos] = np.array([info.get("TimeLimit.truncated", False) for info in infos]) #jmv
            pass
